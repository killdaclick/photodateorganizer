...
InstallELFandFW=Install ELF and FaultViewer
InstallELF=Install ELF only
DescELF=ELF application
DescFW=FaultViewer application
FTDIinstallTxt=USB communications port driver installation will start in a few seconds. During installation the “Digital signature missing” message will appear. The “Continue” button should then be pressed.
AppendELF1installDirInfo=Please do not install into old "ELF 1" installation directory.
NewELF2projectType=Application "ELF 2" is working with new type projects. Old type "ELF 1" projects will be copied and converted to new type when opened for the first time. Old type projects are NOT deleted!